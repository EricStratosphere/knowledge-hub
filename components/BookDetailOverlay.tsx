      ...
      <div className="w-full max-w-[840px] bg-gradient-to-br from-[#1C2230] via-[#121620] to-[#0A0D14] border border-white/[0.08] rounded-2xl p-6 md:p-10 shadow-[0_30px_90px_rgba(0,0,0,0.9)] relative overflow-hidden flex flex-col md:min-h-[460px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10 items-stretch flex-1 z-10">
          ...
          <div className="col-span-1 md:col-span-8 flex flex-col justify-start">
            ...
            {activeTab === 'details' ? (
              /* --- DETAILS PANEL VIEW --- */
              <div className="flex flex-col h-full max-h-[380px] animate-fade-in text-left">
                {/* 1. Header & Summary Info (Capped height) */}
                <div className="flex-shrink-0 max-h-[140px] overflow-y-auto pr-1 scrollbar-thin mb-1">
                  {/* Book Title */}
                  <h3 className="font-luxury-serif text-2xl md:text-3xl font-light text-white leading-tight tracking-wide mb-2">
                    {book.book_title}
                  </h3>
                ...
                {/* 2. Divider Line */}
                <div className="border-t border-white/[0.06] my-3 flex-shrink-0" />

                {/* 3. Scrollable Review Feed */}
                <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin space-y-4">
                  
                  {/* Review Input Form Component */}
                  <div className="bg-white/[0.015] border border-white/[0.05] rounded-xl p-3.5 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-semibold text-white/40 uppercase tracking-widest">
                        Rate this book
                      </span>
                      {/* 5 star selection buttons */}
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <button
                            key={star}
                    ...
                    <div className="space-y-2">
                      <textarea
                        ...
                        className="w-full bg-transparent border-b border-white/10 focus:border-white focus:outline-none transition-all duration-200 py-1.5 text-white placeholder-white/20 text-xs resize-none h-12 focus:ring-0"
                      />
                  ...
                  <div className="space-y-4">
                    {reviewsList.map((review) => (
                      <div key={review._id} className="flex gap-3 text-left">
                        {/* Avatar block */}
                        <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-xs font-semibold text-white/50 select-none flex-shrink-0">
                        ...
                        <div className="flex-1 space-y-1">
                          ...
                          <div className="flex items-center gap-0.5">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <svg
                                key={star}
                                className={`w-3 h-3 ${star <= review.rating ? 'text-yellow-400 fill-yellow-400' : 'text-white/10'}`}
                                viewBox="0 0 20 20"
                                fill="currentColor"
                              >
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
